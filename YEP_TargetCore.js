...
// 本插件修改了makeTargets，makeCustomTargets和getRandomTargets函数，使插件支持技能随机选择敌人时候不重复选择同一个敌人。By Misuqian 20170212
//=============================================================================
  ...
  if (this.isForEval()) {
    ...
    for (var i = 0; i < number; ++i) {
	  ...
	  for(var j = 0;j < group.length; j++){
	  	      var uni = group[j];
	  		  if(uni.name() === member.name()){
	  			 //从数组中删除这个对象，从而避免了随机重复
	  		  	 group.splice(j, 1);
	  			 break;
	  		  }
	  }
...
Game_Action.prototype.getRandomTargets = function(number, unit) {
  ...
  //深拷贝unit存活的对象
  var wholeUnit = [];
  for (var i = 0; i < unit.aliveMembers().length; ++i) {
	  wholeUnit.push(unit.aliveMembers()[i]);
  }
  var num = number;
  //判断人数，防止溢出重复
  if(wholeUnit.length < number){
	  num = wholeUnit.length;
  }
  for (var i = 0; i < num; ++i) {
	  if(wholeUnit.length <= 0){
		  break;
	  }
	  var index = Math.floor(Math.random() * wholeUnit.length);
	  var obj = wholeUnit[index];
      targets.push(obj);
	  for(var j = 0;j < wholeUnit.length; ++j){
	  	      var uni = wholeUnit[j];
	  		  if(uni.name() === obj.name()){
	  			 //从数组中删除这个对象，从而避免了随机重复
// console.log("删除了"+obj.name());
	  		  	 wholeUnit.splice(j, 1);
	  			 break;
	  		  }
	  }
  }
